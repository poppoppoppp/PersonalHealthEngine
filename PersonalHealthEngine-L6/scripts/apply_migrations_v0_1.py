...
def main():
    ...
    applied = []
    try:
        has_registry = db.execute("SELECT 1 FROM sqlite_master WHERE type='table' AND name='schema_migrations'").fetchone()
        existing = ({row["version"]: row for row in db.execute("SELECT * FROM schema_migrations")} if has_registry else {})
        for version, name, path in migration_files:
            ...
            try:
                ...
                db.execute("INSERT INTO schema_migrations (version,name,applied_at_utc,checksum_sha256) VALUES (?,?,?,?)", (version, name, utc_now(), checksum))
        ...
        print(json.dumps({"status": "PASS", "schema_version": db.execute("PRAGMA user_version").fetchone()[0], "applied": applied, "migration_count": len(migration_files)}, indent=2))
    finally:
        db.close()
