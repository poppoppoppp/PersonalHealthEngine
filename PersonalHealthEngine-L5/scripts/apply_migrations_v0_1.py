...
def main():
    ...
    try:
        ...
        existing = (
            {row["version"]: row for row in db.execute("SELECT * FROM schema_migrations")}
            if has_registry
            else {}
        )
        for version, name, path in migration_files:
            ...
            try:
                ...
                db.execute(
                    "INSERT INTO schema_migrations (version,name,applied_at_utc,checksum_sha256) VALUES (?,?,?,?)",
        ...
        if integrity != "ok" or foreign_key_errors:
            raise RuntimeError(f"post-migration integrity failure: {integrity}, fk={len(foreign_key_errors)}")
        result = {
            "status": "PASS",
